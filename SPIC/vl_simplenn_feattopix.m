function res = vl_simplenn_feattopix(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'l1loss'
      res(i+1).x = l1loss(res(i).x, l.class) ;
    case 'l2loss'
      res(i+1).x = l2loss(res(i).x, l.class) ;
    case 'l2hist'
      res(i+1).x = l2hist(res(i).x, l.class) ;
    case 'MSEloss'
      res(i+1).x = vl_nnMSEloss(res(i).x,l.class);
    case 'edgeloss'
      res(i+1).x = vl_edgeloss(res(i).x,l.class);  
    case 'relu'
      res(i+1).x = vl_nnrelu(res(i).x) ;
    case 'brelu'
      res(i+1).x = vl_nnbrelu(res(i).x) ;
    case 'sigmoid'
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'l1loss'
        res(i).dzdx = l1loss(res(i).x, l.class,res(i+1).dzdx) ;
      case 'l2loss'
        res(i).dzdx = l2loss(res(i).x, l.class,res(i+1).dzdx) ;
      case 'l2hist'
        res(i).dzdx = l2hist(res(i).x, l.class,res(i+1).dzdx) ;
      case 'MSEloss'
        res(i).dzdx = vl_nnMSEloss(res(i).x,l.class,res(i+1).dzdx);
      case 'edgeloss'
        res(i).dzdx = vl_edgeloss(res(i).x,l.class,res(i+1).dzdx);
      case 'relu'
        if ~isempty(res(i).x)
       ...
       case 'brelu'
        if ~isempty(res(i).x)
          res(i).dzdx = vl_nnbrelu(res(i).x, res(i+1).dzdx) ;
        else
          % if res(i).x is empty, it has been optimized away, so we use this
          % hack (which works only for ReLU):
          res(i).dzdx = vl_nnbrelu(res(i+1).x, res(i+1).dzdx) ;
        end
      case 'sigmoid'
